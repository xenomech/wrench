...
function parseInput(
  ...
  try {
    ...
    const errorInfo = extractErrorPosition(err, format, input);
    return {
      success: false,
...
export function convert(
  ...
  if (
    ...
    return {
      success: false,
      error:
        "TOML only supports objects at the root level. Arrays, strings, numbers, and null cannot be represented.",
    };
  }
  ...
  try {
    ...
    return {
      success: false,
      error: `Cannot convert to ${to.toUpperCase()}: ${(e as Error).message}`,
    };
  }
}
