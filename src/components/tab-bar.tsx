...
export function useActiveTab(): { id: string; theme: TabTheme } {
  ...
  return {
    id: tab?.id ?? "generators",
    theme: tab?.theme ?? TAB_THEMES.generators!,
  };
}
...
export function TabBar() {
  const pathname = usePathname();
  const activeTabId =
    tabs.find((t) => pathname.startsWith(t.href))?.id ?? "generators";
  const soundEnabled = useSoundStore((s) => s.enabled);
  const [playClick] = useSound(click002Sound, { volume: 0.3, soundEnabled });
